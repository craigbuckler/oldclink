...
(function() {
  ...
  // unsupported browser
  if (!oc.name || !FormData || !document.body.classList) return;

  // find all forms
  var form = oc.lib.queryAll('form.validator');
  ...
  function Validator(form, cfg) {
    ...
    this.cfg = cfg;
    this.form = form;
    this.error = oc.lib.query(cfg.errorElement);
    this.postback = {};
    this.submitting = false;

    // initialise
    oc.lib.ajax(
      this.form.action + this.cfg.methodInit + '/' + oc.name,
      function(err, url, data) {

        // invalid request?
        if (err || !data || !data._e) return;

        // initialise form
        this.postback = data;

        this.form.action = this.form.action + this.cfg.methodSubmit + '/' + oc.name;
  ...
  Validator.prototype.submit = function(e) {
    ...
    // Ajax request
    oc.lib.ajax(this.form, complete.bind(this), append.bind(this));
